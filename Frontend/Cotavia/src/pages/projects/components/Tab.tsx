...
export default function(props: useProps) {
    ...
    const {setPath, setData, currentPath} = useContext<project>(projectContext)

    const updateData = () => {
        setPath(props.path)
        setData(getFile(props.root, props.path))
    }
    const del = () => {
        const x = props.tabs[0] != props.path
        if (props.tabs.length > 1) {
            setPath(x? props.tabs[0]: props.tabs[1])
            setData(getFile(props.root, x? props.tabs[0]: props.tabs[1]))
        }
        else
            setPath("")   
    ...
    return (
        <div className={'bg-white backdrop-blur-xl gap-2 rounded-lg bg-opacity-5 h-full p-2 min-w-32 max-w-60 w-fit flex justify-between items-center '
                ...
                <div className='max-w-52 overflow-clip w-[90%] hover:cursor-default' onClick={updateData}>
                        {x[x.length - 1]}
                </div>
